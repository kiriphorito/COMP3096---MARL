...
# Note: This file is modified from env/sc2_env.py.
# Changes are marked with `RGP`.
#
# SC2MarlEnv is an environment that allows agents to make multiple actions per time-step.
#
# Important notes:
# Support for multiple *independent* agents with their own controllers (i.e. 2 agents playing opposing sides)
# is DISABLED in this environment in favour of allowing a *single agent* to take multiple steps.
# Search for `RGP` for more information.
#
# Summary of changes:
# - Original behaviour:
# Each controller has one observation and one action.
# Observation, controller and action are `zip`-ed up.
# This is problematic because each controller (agent) is expected to have exactly one action.
# Otherwise the array sizes of `observation` and `controller` will no longer line up with `actions` for use with zip.
#
# - New behaviour:
# When there are multiple actions, associate them with agent 0's controller and observation.
# Directly construct a protobuf object (this is originally done in RemoteController) and send the request via controller 0.

"""RGP: A Starcraft II multi-agent environment."""
...
class SC2MarlEnv(environment.Base):
  """RGP: A Starcraft II multi-agent environment.
  ...
  def step(self, actions):
    ...
    if (len(actions) > 1):
      transformed_actions = [self._features.transform_action(self._obs[0].observation, a) for a in actions]
