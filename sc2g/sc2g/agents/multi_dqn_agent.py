...
# Environment
import sc2g
from sc2g.env.sc2env_wrapper import SC2EnvWrapper
from sc2g.env.movement import MultiMovementIndexedEnv
# Algorithm
...
def train():
    print("making sc2")
    # Make the SC2 Environment
    sc2_env = SC2EnvWrapper(
        num_agents=2,
    ...
    # Make 2 environments
    env1 = MultiMovementIndexedEnv(sc2_env, id=0, unit_to_move=0)
    env2 = MultiMovementIndexedEnv(sc2_env, id=1, unit_to_move=1)

    print("Made environments")

    t1 = threading.Thread(target=go_train, args=(env1,))
    t2 = threading.Thread(target=go_train, args=(env2,))
